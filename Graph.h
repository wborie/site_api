...
template <typename Node>
std::vector<std::string> Graph<Node>::dijkstra(Node startNode) {
	std::vector<std::string> result;
	bool nodeExists = false;
	for(int i = 0; i < capacity; i++) {
		if (nodeList[i]->node == startNode) {
			nodeExists = true;
			break;
		}
	}
	if (!nodeExists) return result;

	NodeContainer** parent = new NodeContainer*[capacity];
	double* distance = new double[capacity];
	bool* inQueue = new bool[capacity];
	Edge** traversalOrder = new Edge*[numEdges];
	std::string parentOverrideTracker = "";
	typedef std::priority_queue<NodeContainer*, std::vector<NodeContainer*>, DijkstraNodeContainerComparator> p_queue;
	p_queue queue((DijkstraNodeContainerComparator(distance)));

	for(int i = 0; i < capacity; i++) {
		parent[i] = nullptr;
		distance[i] = DBL_MAX;
		inQueue[i] = true;
		if (nodeList[i]->node == startNode) distance[i] = 0;
		queue.push(nodeList[i]);
	}
	for(int i = 0; i < numEdges; i++) {
		traversalOrder[i] = nullptr;
	}

	int traversalCount = 0;
	while (!queue.empty()) {
		NodeContainer* currentNodeContainer = queue.top();
		int currentNodeContainerIndex = currentNodeContainer->nodeListIndex;
		queue.pop();
		inQueue[currentNodeContainer->nodeListIndex] = false;
		std::vector<Edge*> adjacentNodes = currentNodeContainer->edges;
		for(unsigned int i = 0; i < adjacentNodes.size(); i++) {
			// if (!inQueue[adjacentNodes[i]->to->nodeListIndex]) continue;
			int adjacentNodeIndex = adjacentNodes[i]->to->nodeListIndex;
			double option = distance[currentNodeContainerIndex] + adjacentNodes[i]->weight;
			if (option < distance[adjacentNodeIndex]) {
				traversalOrder[traversalCount] = adjacentNodes[i];
				traversalCount++;
				// Check here if distance being overriden is not infinite, if so, a highlight needs to be overriden
				// pass along an array with values of either null or edge to be overridden
				if (distance[adjacentNodeIndex] != DBL_MAX) {
					parentOverrideTracker += adjacentNodes[i]->to->node + "-" + parent[adjacentNodeIndex]->node + ",";
				}
				else parentOverrideTracker += ",";
				distance[adjacentNodeIndex] = option;
				parent[adjacentNodeIndex] = currentNodeContainer;
				queue.push(adjacentNodes[i]->to);
			}
		}
	}
	...
	for(int i = 0; i < capacity; i++) {
		if (!nodeList[i]->set) continue;
		nodes += nodeList[i]->node + ",";
		if (parent[i] != nullptr) parents += parent[i]->node + ",";
		else parents += ",";
		if (!(distance[i] == DBL_MAX)) distances += std::to_string(distance[i]) + ","; 
		else distances += "Infinity,";

		// result += nodeList[i]->node;
		// if (parent[i] != nullptr) result += " - " + parent[i]->node;
		// if (!(distance[i] == DBL_MAX)) result += " * " + std::to_string(distance[i]);
		// else result += " * Infinity";
		// result += "\n";
		// std::cout << nodeList[i]->node << " - " << parent[i]->node << std::endl;
	}

	std::string traversalOrders = "";
	for(int i = 0; i < numEdges; i++) {
		if (traversalOrder[i] != nullptr) traversalOrders += traversalOrder[i]->from->node + "-" + traversalOrder[i]->to->node + ",";
	}

	nodes.erase(nodes.size() - 1); // Remove the last comma
	parents.erase(parents.size() - 1);
	distances.erase(distances.size() - 1);
	traversalOrders.erase(traversalOrders.size() - 1);
	parentOverrideTracker.erase(parentOverrideTracker.size() - 1);
	result.push_back(nodes);
	result.push_back(parents);
	result.push_back(distances);
	result.push_back(traversalOrders);
	result.push_back(parentOverrideTracker);
